...
> **现状更新（2026-09-05 文档基本面）**：产品/功能文档已回写对齐仓库 `main` @ `35888f6`（见 `docs/功能清单.md`、`Product-Spec` SCOPE-018 计划任务表述、RISK→自动排障）。**部署侧**：内部机曾于 09-03 以 `36ed915` + 计划任务重启生效 SCOPE-019；若机器未 `git pull` 至含 docs 的最新 main，不影响运行，但文档与磁盘代码可能再次分叉。SCOPE-020 前端列表页（P1 排后）仍 ⏳。
